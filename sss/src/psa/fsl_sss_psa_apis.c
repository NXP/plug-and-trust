...
static psa_key_type_t sss_to_psa_key_type(se_sss_cipher_type_t cipherType,
                                           ...
                                           size_t keyBitLen);
static psa_key_usage_t sss_to_psa_key_usage(se_sss_access_permission_t accessRights,
                                             sss_mode_t purpose,
...
static psa_algorithm_t sss_to_psa_derive_algorithm(sss_algorithm_t algorithm, sss_mode_t mode);
...
{
    psa_key_id_t psa_id = (psa_key_id_t)((sss_key_id %
        (PSA_KEY_ID_USER_MAX - PSA_KEY_ID_USER_MIN)) + PSA_KEY_ID_USER_MIN);
    ...
    if (psa_get_key_attributes(psa_id, &attrs) == PSA_SUCCESS) {
        ...
        if (status != PSA_SUCCESS && status != PSA_ERROR_INVALID_HANDLE) {
            LOG_E("Failed to destroy existing key during collision handling: %d", status);
    ...
    size_t expected_key_bits)
{
    ...
    size_t expected_raw_len = (expected_key_bits / 8) * 2 + 1;  // 65 for P-256 (0x04 + x + y)
    ...
    ret = mbedtls_ecp_point_write_binary(&ecp->MBEDTLS_PRIVATE(grp),
    ...
    size_t expected_key_bits)
{
    ...
    ret = mbedtls_mpi_write_binary(&ecp->MBEDTLS_PRIVATE(d), raw_key, expected_raw_len);
    if (ret != 0) {
...
static sss_status_t convert_key_to_psa_format(
        ...
        if (key_data_len > 65 && key_data[0] == 0x30) {
            *psa_key_len = output_buffer_size;
            retval = extract_ecc_public_key_from_x509(
                ...
                output_buffer, psa_key_len, key_bits);
 ...
 */
static sss_status_t add_ecc_header_psa(uint8_t *key,
                                             size_t *keylen,
                                             uint8_t **key_buf,
                                             size_t *key_buflen,
                                             psa_key_type_t key_type,
...
static sss_status_t sss_mbedtls_import_key_with_algorithm(
    ...
    psa_key_usage_t additional_usage)
{
    ...
    if (keyObject->psa_key_id != PSA_KEY_ID_NULL) {
        ...
        if (psa_status == PSA_SUCCESS) {
            ...
            if (existing_alg == algorithm) {
                LOG_D("Reusing existing PSA key ID: %lu", (unsigned long)keyObject->psa_key_id);
    ...
    psa_key_type_t key_type = sss_to_psa_key_type(
        ...
        (sss_key_part_t)keyObject->objectType,
        keyObject->keyBitLen);
    ...
    retval = convert_key_to_psa_format(
        keyObject->contents,
        keyObject->contents_size,
        key_type,
        keyObject->keyBitLen,
        raw_key_buffer,
        sizeof(raw_key_buffer),
        &key_data_to_import,
        &key_data_len);
...
/* Map once here — all PSA API calls use psa_key_id which is always valid */
    keyObject->psa_key_id            = (options == kKeyObject_Mode_Persistent)
                                        ? sss_app_key_id_to_psa(keyId) : PSA_KEY_ID_NULL;
...
sss_status_t sss_mbedtls_key_object_set_user(sss_mbedtls_object_t *keyObject, uint32_t user, uint32_t options)
{
    ...
    ENSURE_OR_GO_EXIT((keyObject->accessRights & kAccessPermission_SE_SSS_ChangeAttributes));
...
// LCOV_EXCL_START
sss_status_t sss_mbedtls_key_object_set_eccgfp_group(sss_mbedtls_object_t *keyObject, se_sss_eccgfp_group_t *group)
{
    sss_status_t retval = kStatus_SSS_Success;
...
void sss_mbedtls_key_object_free(sss_mbedtls_object_t *keyObject)
{
    if (keyObject != NULL) {
        ...
        if (keyObject->contents != NULL && keyObject->contents_must_free) {
            memset(keyObject->contents, 0, keyObject->contents_size); // Zero before free
    ...
    size_t optionsLen)
{
    ...
    ENSURE_OR_GO_CLEANUP((keyObject->accessRights & kAccessPermission_SE_SSS_Write));
    ...
    void *options)
{
    ...
    psa_key_usage_t usage = sss_to_psa_key_usage(
    ...
    size_t *pKeyBitLen)
{
    ...
    ENSURE_OR_GO_CLEANUP((keyObject->accessRights & kAccessPermission_SE_SSS_Read));
    ...
    default:
        ...
        if (PSA_KEY_TYPE_IS_ECC(key_type) &&
            ...
            psa_status = psa_export_public_key(keyObject->psa_key_id,
                                               raw_key, raw_key_len,
                                               &exported_length);
            ...
            psa_status = psa_export_key(keyObject->psa_key_id, key, *keyLen, &exported_length);
...
sss_status_t sss_mbedtls_key_store_open_key(sss_mbedtls_key_store_t *keyStore, sss_mbedtls_object_t *keyObject)
{
    ...
    if (keyObject->keyMode == kKeyObject_Mode_Persistent) {
        ...
        if (psa_status == PSA_SUCCESS) {
            ...
            LOG_W("Persistent key %u (mapped from 0x%08X) not found",
                  key_id, keyObject->keyId);
        }
    }
    else {
...
sss_status_t sss_mbedtls_key_store_freeze_key(sss_mbedtls_key_store_t *keyStore, sss_mbedtls_object_t *keyObject)
{
    ...
    if (keyObject != NULL) {
        ...
        keyObject->accessRights &= ~(uint32_t)kAccessPermission_SE_SSS_ChangeAttributes;
    }
...
sss_status_t sss_mbedtls_key_store_erase_key(sss_mbedtls_key_store_t *keyStore, sss_mbedtls_object_t *keyObject)
{
    ...
    ENSURE_OR_GO_CLEANUP(keyStore != NULL);
    ENSURE_OR_GO_CLEANUP(keyObject != NULL);
    ENSURE_OR_GO_CLEANUP((keyObject->accessRights & kAccessPermission_SE_SSS_Delete));
    ...
    sss_mode_t mode)
{
    ...
    context->cipher_op = (psa_cipher_operation_t *)SSS_CALLOC(1, sizeof(psa_cipher_operation_t));
    ...
    *context->cipher_op = psa_cipher_operation_init();
    ...
    retval = sss_mbedtls_import_key_with_algorithm(keyObject, psa_alg,
                                                PSA_KEY_USAGE_ENCRYPT | PSA_KEY_USAGE_DECRYPT);
...
cleanup:
    if (retval != kStatus_SSS_Success && context != NULL && context->cipher_op != NULL) {
    ...
    size_t dataLen)
{
    ...
    if (context->mode == kMode_SSS_Encrypt) {
        psa_status = psa_cipher_encrypt_setup(&op, context->keyObject->psa_key_id, alg);
    }
    else if (context->mode == kMode_SSS_Decrypt) {
        psa_status = psa_cipher_decrypt_setup(&op, context->keyObject->psa_key_id, alg);
    }
    else {
    ...
    /* Process data */
    psa_status = psa_cipher_update(&op,
                                   srcData, dataLen,
                                   destData, output_buffer_size,
                                   &update_out);
    ...
    psa_status = psa_cipher_finish(&op,
                                   destData + update_out,
                                   finish_buffer_size,
                                   &finish_out);
    ...
    size_t *destLen)
{
    ...
    psa_status = psa_cipher_update(context->cipher_op,
        srcData, srcLen, destData, *destLen, &output_length);
    ...
    size_t *destLen)
{
    ...
    if (srcData != NULL && srcLen > 0) {
        psa_status = psa_cipher_update(context->cipher_op,
            srcData, srcLen, destData, *destLen, &update_out);
        if (psa_status != PSA_SUCCESS) {
    ...
    psa_status = psa_cipher_finish(context->cipher_op,
        destData + total_out, *destLen - total_out, &finish_out);
    if (psa_status != PSA_SUCCESS) {
        LOG_E("PSA cipher finish failed: %d", psa_status);
        goto cleanup;
    }
    ...
    size_t *szLeft)
{
     ...
     */
    if (context->mode == kMode_SSS_Encrypt || context->mode == kMode_SSS_Decrypt) {
        psa_status = psa_cipher_encrypt_setup(&op,
            context->keyObject->psa_key_id, PSA_ALG_CTR);
    }
    else {
    ...
    psa_status = psa_cipher_update(&op, srcData, size,
        destData, size + 16, &update_out);
    if (psa_status != PSA_SUCCESS) {
    ...
    psa_status = psa_cipher_finish(&op,
        destData + update_out, 16, &finish_out);
    if (psa_status != PSA_SUCCESS) {
...
{
    switch (algorithm) {
    case kAlgorithm_SSS_SHA1:   return PSA_ALG_SHA_1;
    case kAlgorithm_SSS_SHA224: return PSA_ALG_SHA_224;
    case kAlgorithm_SSS_SHA256: return PSA_ALG_SHA_256;
    case kAlgorithm_SSS_SHA384: return PSA_ALG_SHA_384;
    case kAlgorithm_SSS_SHA512: return PSA_ALG_SHA_512;
    ...
    sss_mode_t mode)
{
    ...
    context->hash_op = (psa_hash_operation_t *)SSS_CALLOC(1, sizeof(psa_hash_operation_t));
    ...
    *context->hash_op = psa_hash_operation_init();
    ...
    switch (algorithm) {
    case kAlgorithm_SSS_SHA1:   context->digestFullLen = 20; break;
    case kAlgorithm_SSS_SHA224: context->digestFullLen = 28; break;
    case kAlgorithm_SSS_SHA256: context->digestFullLen = 32; break;
    case kAlgorithm_SSS_SHA384: context->digestFullLen = 48; break;
    case kAlgorithm_SSS_SHA512: context->digestFullLen = 64; break;
    default:                    context->digestFullLen = 0;  break;
    }

    retval = kStatus_SSS_Success;

cleanup:
    if (retval != kStatus_SSS_Success && context != NULL && context->hash_op != NULL) {
    ...
    size_t *digestLen)
{
    ...
    psa_status = psa_hash_compute(psa_alg, message, messageLen,
        digest, *digestLen, &hash_length);
    ...
    size_t *digestLen)
{
    ...
    psa_status = psa_hash_finish(context->hash_op, digest, *digestLen, &hash_length);
    if (psa_status != PSA_SUCCESS) {
    ...
    sss_mode_t mode)
{
    ...
    context->mac_op = (psa_mac_operation_t *)SSS_CALLOC(1, sizeof(psa_mac_operation_t));
    ...
    size_t *macLen)
{
    ...
    if (context->mode == kMode_SSS_Mac) {
        psa_status = psa_mac_compute(context->keyObject->psa_key_id,
            psa_alg, message, messageLen, mac, *macLen, &mac_length);
        ...
        *macLen = mac_length;
        ...
        psa_status = psa_mac_verify(context->keyObject->psa_key_id,
            psa_alg, message, messageLen, mac, *macLen);
...
cleanup:
    if (retval != kStatus_SSS_Success && context != NULL && context->operation_initialized) {
        psa_mac_abort(context->mac_op);
        *context->mac_op               = psa_mac_operation_init();
        context->operation_initialized = false;
    }
...
sss_status_t sss_mbedtls_mac_finish(sss_mbedtls_mac_t *context,
    uint8_t *mac,
    size_t *macLen)
{
    sss_status_t retval = kStatus_SSS_Fail;
    ...
    if (context->mode == kMode_SSS_Mac) {
        psa_status = psa_mac_sign_finish(context->mac_op, mac, *macLen, &mac_length);
        if (psa_status != PSA_SUCCESS) {
...
static psa_algorithm_t sss_to_psa_asym_algorithm(sss_algorithm_t algorithm)
{
    switch (algorithm) {
    /* ECDSA */
    case kAlgorithm_SSS_ECDSA_SHA1:   return PSA_ALG_ECDSA(PSA_ALG_SHA_1);
    case kAlgorithm_SSS_ECDSA_SHA224: return PSA_ALG_ECDSA(PSA_ALG_SHA_224);
    case kAlgorithm_SSS_ECDSA_SHA256: return PSA_ALG_ECDSA(PSA_ALG_SHA_256);
    case kAlgorithm_SSS_ECDSA_SHA384: return PSA_ALG_ECDSA(PSA_ALG_SHA_384);
    case kAlgorithm_SSS_ECDSA_SHA512: return PSA_ALG_ECDSA(PSA_ALG_SHA_512);
    ...
    size_t *destLen)
{
    ...
    ENSURE_OR_GO_CLEANUP(context->keyObject != NULL);
    ENSURE_OR_GO_CLEANUP(context->keyObject->psa_key_id != PSA_KEY_ID_NULL);
    ENSURE_OR_GO_CLEANUP((context->keyObject->accessRights & kAccessPermission_SE_SSS_Use));
    ...
    psa_status = psa_asymmetric_encrypt(context->keyObject->psa_key_id,
        psa_alg, srcData, srcLen, NULL, 0,
        destData, *destLen, &output_length);
    ...
    size_t *destLen)
{
    ...
    psa_status = psa_asymmetric_decrypt(context->keyObject->psa_key_id,
        psa_alg, srcData, srcLen, NULL, 0,
        destData, *destLen, &output_length);
    ...
    size_t *signatureLen)
{
    ...
    const bool is_ecdsa = PSA_ALG_IS_ECDSA(psa_alg) || psa_alg == PSA_ALG_ECDSA_ANY;
    ...
    if (is_ecdsa) {
        ...
        psa_status = psa_sign_hash(context->keyObject->psa_key_id,
            psa_alg, digest, digestLen, raw_sig, raw_sig_len, &raw_sig_len);
        ...
        psa_status = psa_sign_hash(context->keyObject->psa_key_id,
            psa_alg, digest, digestLen, signature, *signatureLen, signatureLen);
    ...
    size_t signatureLen)
{
    ...
    /* Check if we need DER conversion (ECDSA only) */
    const bool is_ecdsa = PSA_ALG_IS_ECDSA(psa_alg) &&
    ...
    if (is_ecdsa) {
        ...
        psa_status = psa_verify_hash(context->keyObject->psa_key_id,
            psa_alg, digest, digestLen, signature, signatureLen);
    ...
    size_t *hkdfOutputLen)
{
    ...
    if (context->mode == kMode_SSS_HKDF_ExtractExpand) {
        if (saltData != NULL && saltLen > 0) {
            psa_status = psa_key_derivation_input_bytes(&operation,
                PSA_KEY_DERIVATION_INPUT_SALT, saltData, saltLen);
            if (psa_status != PSA_SUCCESS) {
    ...
    psa_status = psa_key_derivation_input_key(&operation,
        PSA_KEY_DERIVATION_INPUT_SECRET, context->keyObject->psa_key_id);
    if (psa_status != PSA_SUCCESS) {
    ...
    if (info != NULL && infoLen > 0) {
        psa_status = psa_key_derivation_input_bytes(&operation,
            PSA_KEY_DERIVATION_INPUT_INFO, info, infoLen);
        if (psa_status != PSA_SUCCESS) {
    ...
    if (derivedKeyObject != NULL) {
        ...
        if (derivedKeyObject->keyMode == kKeyObject_Mode_Persistent) {
            psa_set_key_lifetime(&attributes, PSA_KEY_LIFETIME_PERSISTENT);
            psa_set_key_id(&attributes, sss_app_key_id_to_psa(derivedKeyObject->keyId));
        }

        psa_status = psa_key_derivation_output_key(&attributes, &operation, &derived_key_id);
        psa_reset_key_attributes(&attributes);
        ...
        psa_status = psa_key_derivation_output_bytes(&operation, hkdfOutput, deriveDataLen);
        if (psa_status != PSA_SUCCESS) {
    ...
    uint16_t deriveDataLen)
{
    ...
    return sss_mbedtls_derive_key_go(context,
        saltData, adjustedSaltLen, info, infoLen,
        derivedKeyObject, deriveDataLen, NULL, NULL);
}
    ...
    uint16_t deriveDataLen)
{
    ...
    if (context->mode == kMode_SSS_HKDF_ExtractExpand &&
        ...
        psa_status = psa_key_derivation_input_key(&operation,
            PSA_KEY_DERIVATION_INPUT_SALT, saltKeyObject->psa_key_id);
        if (psa_status != PSA_SUCCESS) {
    ...
    sss_mbedtls_object_t *derivedKeyObject)
{
    ...
    retval = sss_mbedtls_import_key_with_algorithm(otherPartyKeyObject, psa_alg, usage);
    ENSURE_OR_GO_CLEANUP(retval == kStatus_SSS_Success);

    /* Export peer's public key */
    psa_status = psa_export_public_key(otherPartyKeyObject->psa_key_id,
        peer_pub_key, sizeof(peer_pub_key), &peer_pub_key_len);
    if (psa_status != PSA_SUCCESS) {
    ...
    psa_set_key_usage_flags(&attrs,
        PSA_KEY_USAGE_DERIVE | PSA_KEY_USAGE_ENCRYPT |
    ...
    psa_status = psa_import_key(&attrs, shared_secret, shared_secret_len, &derived_key_id);
    psa_reset_key_attributes(&attrs);
