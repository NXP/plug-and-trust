/*
 *
 * Copyright 2026 NXP
 * SPDX-License-Identifier: BSD-3-Clause
 */

#include <nxLog_App.h>
#include <stdbool.h>
#include "se05x_host_gpio.h"
#include "board.h"
#include "FreeRTOS.h"
#include "task.h"
#include "semphr.h"
#include "fsl_gpio.h"
#include "fsl_io_mux.h"

/***********************************************************************************************************************
 * Definitions
 **********************************************************************************************************************/
#if defined(CONFIG_SE05X_HOST_GPIO)
#define SE05X_NOTIFY_GPIO_PORT          0U
#define SE05X_NOTIFY_GPIO_PIN           18U

#define GPIO_HANDLER_TASK_PRIORITY      (configMAX_PRIORITIES - 2)
#define GPIO_HANDLER_TASK_STACK_SIZE    512

/***********************************************************************************************************************
 * Variables
 **********************************************************************************************************************/
static TaskHandle_t g_GpioHandlerTaskHandle = NULL;
static SemaphoreHandle_t g_GpioInterruptSemaphore = NULL;
static volatile bool g_TaskRunning = false;
static TaskHandle_t g_DeinitWaitingTask = NULL;

/***********************************************************************************************************************
 * Prototypes
 **********************************************************************************************************************/
static void SE05X_NotifyTask(void *pvParameters);
static void SE05X_HandleFallingEdge(void);

/***********************************************************************************************************************
 * Code
 **********************************************************************************************************************/

/*!
 * @brief GPIO interrupt handler (ISR context) for RW61X
 */
void GPIO_INTA_IRQHandler(void)
{
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;

    /* Check if our specific pin triggered the interrupt */
    if (GPIO_PortGetInterruptStatus(GPIO, SE05X_NOTIFY_GPIO_PORT, 0U))
    {
        /* Clear the interrupt flag immediately to prevent re-triggering */
        GPIO_PortClearInterruptFlags(GPIO, SE05X_NOTIFY_GPIO_PORT, 0U, (1U << SE05X_NOTIFY_GPIO_PIN));

        /* Signal the handler task that an interrupt occurred */
        if (g_GpioInterruptSemaphore != NULL)
        {
            xSemaphoreGiveFromISR(g_GpioInterruptSemaphore, &xHigherPriorityTaskWoken);
        }
    }
    /* This ensures the handler task runs as soon as possible after the ISR completes */
    portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
}

/*!
 * @brief Handle falling edge detection in task context
 */
static void SE05X_HandleFallingEdge(void)
{
    /* Restart Matter application to handle the SE05x notification */
    LOG_I("RW61X: SE05X notification received, restarting application...");

    /* Add delay before reset to allow any pending operations to complete can be configurable to platforms*/
    SDK_DelayAtLeastUs(90000, CLOCK_GetFreq(kCLOCK_CoreSysClk));

    /* Perform system reset - this is a safe operation in task context */
    NVIC_SystemReset();

    /* Should never reach here after reset */
    while(1);
}

/*!
 * @brief SE05X GPIO interrupt handler task
 */
static void SE05X_NotifyTask(void *pvParameters)
{
    (void)pvParameters;

    while (g_TaskRunning)
    {
        /* Block waiting for the semaphore from ISR */
        if (xSemaphoreTake(g_GpioInterruptSemaphore, pdMS_TO_TICKS(100)) == pdTRUE)
        {
            /* Semaphore received - an interrupt occurred */
            if (!g_TaskRunning)
            {
                LOG_I("RW61X: SE05X GPIO deinit called");
                break;
            }
            /* Process the interrupt event in task context */
            SE05X_HandleFallingEdge();
        }
    }
    if (g_DeinitWaitingTask != NULL)
    {
        xTaskNotifyGive(g_DeinitWaitingTask);
    }
    g_GpioHandlerTaskHandle = NULL;
    vTaskDelete(NULL);
}

#endif

/*!
 * @brief Initialize SE05X GPIO notification system for RW61X
 * @return NULL on success, non-NULL on failure (to match void* signature)
 */
void * se05x_host_gpio_notification_monitor_init(void * arg)
{
    (void)arg;  // Unused parameter
#if defined(CONFIG_SE05X_HOST_GPIO)
    gpio_pin_config_t gpio_config = {
        .pinDirection = kGPIO_DigitalInput,
        .outputLogic = 0U
    };

    gpio_interrupt_config_t int_config = {
        .mode = kGPIO_PinIntEnableEdge,
        .polarity = kGPIO_PinIntEnableLowOrFall
    };

    /* Create binary semaphore for interrupt signaling */
    g_GpioInterruptSemaphore = xSemaphoreCreateBinary();
    if (g_GpioInterruptSemaphore == NULL)
    {
        LOG_E("RW61X: Failed to create GPIO interrupt semaphore");
        return (void *)(intptr_t)-1;
    }

    /* RW61X: Configure IO MUX for GPIO18 (notification pin) */
    IO_MUX_SetPinMux(IO_MUX_GPIO18);

    /* Initialize GPIO pin as input */
    GPIO_PinInit(GPIO, SE05X_NOTIFY_GPIO_PORT, SE05X_NOTIFY_GPIO_PIN, &gpio_config);

    /* Configure interrupt for falling edge detection */
    GPIO_SetPinInterruptConfig(GPIO, SE05X_NOTIFY_GPIO_PORT, SE05X_NOTIFY_GPIO_PIN, &int_config);

    /* Enable the interrupt for this specific pin */
    GPIO_PinEnableInterrupt(GPIO, SE05X_NOTIFY_GPIO_PORT, SE05X_NOTIFY_GPIO_PIN, 0U);

    /* Enable the GPIO interrupt in NVIC */
    EnableIRQ(GPIO_INTA_IRQn);
    NVIC_SetPriority(GPIO_INTA_IRQn, 3);

    /* Set task running flag before creating the task */
    g_TaskRunning = true;

    /* Create the interrupt handler task */
    if (xTaskCreate(SE05X_NotifyTask,
                    "SE05X_IntHandler",
                    GPIO_HANDLER_TASK_STACK_SIZE,
                    NULL,
                    GPIO_HANDLER_TASK_PRIORITY,
                    &g_GpioHandlerTaskHandle) != pdPASS)
    {
        LOG_E("RW61X: Failed to create SE05X GPIO interrupt handler task");

        /* Cleanup on failure */
        DisableIRQ(GPIO_INTA_IRQn);
        GPIO_PinDisableInterrupt(GPIO, SE05X_NOTIFY_GPIO_PORT, SE05X_NOTIFY_GPIO_PIN, 0U);
        vSemaphoreDelete(g_GpioInterruptSemaphore);
        g_GpioInterruptSemaphore = NULL;
        g_TaskRunning = false;
        return (void *)(intptr_t)-1;
    }
#else
    LOG_I("RW61X: CONFIG_SE05X_HOST_GPIO not defined, GPIO notification disabled");
    return (void *)(intptr_t)-1;
#endif
    return NULL;  // Success
}

/*!
 * @brief Deinitialize SE05X GPIO notification system for RW61X
 */
void se05x_host_gpio_notification_monitor_deinit(void)
{
#if defined(CONFIG_SE05X_HOST_GPIO)
    if (!g_TaskRunning)
    {
        return;
    }

    /* Stop the task */
    g_TaskRunning = false;

    g_DeinitWaitingTask = xTaskGetCurrentTaskHandle();

    /* Disable the interrupt at NVIC level first */
    DisableIRQ(GPIO_INTA_IRQn);

    /* Disable the GPIO pin interrupt */
    GPIO_PinDisableInterrupt(GPIO, SE05X_NOTIFY_GPIO_PORT, SE05X_NOTIFY_GPIO_PIN, 0U);

    /* Clear any pending interrupts */
    GPIO_PortClearInterruptFlags(GPIO, SE05X_NOTIFY_GPIO_PORT, 0U, (1U << SE05X_NOTIFY_GPIO_PIN));

    /* Give semaphore to unblock task if waiting */
    if (g_GpioInterruptSemaphore != NULL)
    {
        xSemaphoreGive(g_GpioInterruptSemaphore);

        /* Wait for task to exit */
        if (ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(500)) == 0)
        {
            if (g_GpioHandlerTaskHandle != NULL)
            {
                vTaskDelete(g_GpioHandlerTaskHandle);
                g_GpioHandlerTaskHandle = NULL;
            }
        }
        vSemaphoreDelete(g_GpioInterruptSemaphore);
        g_GpioInterruptSemaphore = NULL;
    }
    g_DeinitWaitingTask = NULL;

    LOG_I("RW61X: SE05X GPIO notification deinitialized");
#else
    LOG_I("RW61X: CONFIG_SE05X_HOST_GPIO not defined, GPIO notification deinit skipped");
#endif
}

int se05x_host_gpio_power_init()
{
    LOG_I("se05x_host_gpio_power_init - do nothing");
    return 0;
}

int se05x_host_gpio_power_deinit()
{
    LOG_I("se05x_host_gpio_power_deinit - do nothing");
    return 0;
}

int se05x_host_gpio_power_set(bool is_high)
{
#if defined(CONFIG_SE05X_HOST_GPIO)
    bool value = false;
#if CONFIG_SE05X_BOARD_H2
    value = (is_high == true) ? false : true;
#else
    value = is_high;
#endif
    GPIO_PinWrite(GPIO, 0U, 27U, (uint8_t) value);
    /* Add delay to allow SE05x to power up and initialize before I2C communication */
    if (is_high)
    {
        vTaskDelay(pdMS_TO_TICKS(1));  /* 1ms delay for SE05x power-up */
    }
#else
    LOG_I("RW61X: CONFIG_SE05X_HOST_GPIO not defined");
#endif
    return 0;
}
