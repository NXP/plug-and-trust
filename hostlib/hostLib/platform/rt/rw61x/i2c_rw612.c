...
/*
 * I2C implmentation for ICs related to RW61x Family
 */

#include <board.h>

#if defined(FSL_FEATURE_SOC_I2C_COUNT) && FSL_FEATURE_SOC_I2C_COUNT > 0 && defined(FRDM_RW612)
#include "fsl_clock.h"
#include "fsl_i2c.h"
#include "i2c_a7.h"
...
#define DEBUG_PRINT_KINETIS_I2C(Operation, status)                                                                                 \
    ...
    else if (result == kStatus_I2C_Addr_Nak)                                                                                       \
...
#if defined(SDK_OS_FREE_RTOS) && SDK_OS_FREE_RTOS == 1
    NVIC_SetPriority(AX_I2CM_IRQN, 2);
    EnableIRQ(AX_I2CM_IRQN);
    I2C_RTOS_Init(&gmaster_rtos_handle, AX_I2CM, &masterConfig, I2C_MASTER_CLOCK_FREQUENCY);
#else
    I2C_MasterInit(AX_I2CM, &masterConfig, I2C_MASTER_CLOCK_FREQUENCY);
#endif
    return I2C_OK;
}
